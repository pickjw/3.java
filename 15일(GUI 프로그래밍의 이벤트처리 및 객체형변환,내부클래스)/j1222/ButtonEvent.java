...
public class ButtonEvent extends JFrame implements ActionListener{
	...
	public ButtonEvent() {
		...
		b3=new JButton("백");
		b4=new JButton("점프");
		tf1=new JTextField("");//빈문자열에 해당하는 텍스트입력창
		add(b1); add(b2);  add(b3); add(b4); add(tf1);
		//이벤트연결
	...
	//버튼을 클릭할때 자동적으로 호출(ActionEvent e->이벤트 발생 컴포넌트 정보)
	//1.이벤트를 발생시킨 이름을 얻어올 수 있다.->e.getActionCommand();
